...
import (
	...
	websitefake "github.com/normalzzz/clientgo-learning/chapter7/pkg/generated/clientset/versioned/fake"
...
func TestSyncHandlerAddsFinalizerBeforeReconciling(t *testing.T) {
	website := &appsv1alpha1.Website{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "demo",
			Namespace: "default",
	...
	if len(updatedWebsite.Finalizers) != 1 || updatedWebsite.Finalizers[0] != websiteFinalizer {
		t.Fatalf("finalizers = %v, want [%q]", updatedWebsite.Finalizers, websiteFinalizer)
	}
	if len(website.Finalizers) != 0 {
		t.Fatalf("informer cache object was mutated: finalizers = %v", website.Finalizers)
	}
	if actions := kubeClient.Actions(); len(actions) != 0 {
		t.Fatalf("unexpected child resource actions before finalizer update is observed: %v", actions)
	}
}

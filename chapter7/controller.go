...
import (
	...
	appsv1alpha1 "github.com/normalzzz/clientgo-learning/chapter7/pkg/apis/apps/v1alpha1"
	versioned "github.com/normalzzz/clientgo-learning/chapter7/pkg/generated/clientset/versioned"
	websiteinformers "github.com/normalzzz/clientgo-learning/chapter7/pkg/generated/informers/externalversions/apps/v1alpha1"
	websitelisters "github.com/normalzzz/clientgo-learning/chapter7/pkg/generated/listers/apps/v1alpha1"
...
const (
	...
	websiteFinalizer = "apps.clientgo-learning.io/website-finalizer"
)
...
func (c *Controller) syncHandler(ctx context.Context, key string) error {
	...
	// 只为未进入删除流程的 Website 添加 finalizer。更新后等待 informer
	// 推送新版本，避免后续使用旧 resourceVersion 更新 status。
	if website.GetDeletionTimestamp() == nil && !controllerutil.ContainsFinalizer(website, websiteFinalizer) {
		updated := website.DeepCopy()
		controllerutil.AddFinalizer(updated, websiteFinalizer)
		if _, err := c.websiteClient.AppsV1alpha1().Websites(namespace).Update(ctx, updated, metav1.UpdateOptions{}); err != nil {
			return fmt.Errorf("add finalizer to Website %s/%s: %w", namespace, name, err)
		}
		return nil
	}

	// 当 Website 对象被删除时，检查是否包含 finalizer，如果包含则删除 Deployment 和 Service，并移除 finalizer。
	if website.GetDeletionTimestamp() != nil {
		if controllerutil.ContainsFinalizer(website, websiteFinalizer) {
			// 删除 Deployment 和 Service
			if err := c.kubeClient.AppsV1().Deployments(namespace).Delete(ctx, name, metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
				return err
			}
			if err := c.kubeClient.CoreV1().Services(namespace).Delete(ctx, name, metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
				return err
			}
			// 移除 finalizer
			updated := website.DeepCopy()
			controllerutil.RemoveFinalizer(updated, websiteFinalizer)
			_, err := c.websiteClient.AppsV1alpha1().Websites(namespace).Update(ctx, updated, metav1.UpdateOptions{})
			return err
		}
		return nil
	}
